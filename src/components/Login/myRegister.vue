      ...
      <el-form ref="registerForm" :model="registerForm" :rules="registerRules" label-width="100px">
        ...
        <!-- <el-row>
          <el-form-item label="头像上传" prop="avatar">
            <el-upload action="/upload" :on-success="handleAvatarUpload" :before-upload="beforeAvatarUpload"
    ...
    submitForm() {
      ...
      this.instance.post('/user/code',formData,{headers:{'Content-Type':'application/x-www-form-urlencoded'}}).then((response) => {
        if (response.data.code === 200) {
          ...
          this.instance.post('/user/register', this.registerForm).then((response) => {
            if (response.data.code === 200) {
